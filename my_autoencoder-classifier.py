...
with graph.as_default():
  ...
  # Variables.
  weights_hidden0 = tf.Variable(tf.truncated_normal([image_size * image_size, nHidden],stddev=0.01))
  weights_hidden1 = tf.Variable(tf.truncated_normal([nHidden, nHidden],stddev=0.01))
  weights = tf.Variable(tf.truncated_normal([nHidden, image_size*image_size],stddev=0.01))
  biases_hidden0 = tf.Variable(tf.zeros([nHidden]))
  biases_hidden1 = tf.Variable(tf.zeros([nHidden]))
  biases = tf.Variable(tf.zeros([image_size*image_size]))
  
  # Training computation.
  hidden0_comp=tf.matmul(tf_train_dataset, weights_hidden0)
  hidden0 = tf.nn.sigmoid(tf.mul(hidden0_comp  + biases_hidden0,8))
  hidden1_comp=tf.matmul(weights_hidden0, weights_hidden1)
  hidden1 = tf.nn.sigmoid(tf.mul(hidden1_comp  + biases_hidden1,8))
  ...
  valid_comp=tf.matmul(tf.nn.sigmoid(tf.mul(tf.matmul(tf_valid_dataset, weights_hidden0)  + biases_hidden0,8)))
  valid_output_units=tf.nn.sigmoid(tf.matmul(tf.nn.sigmoid(tf.mul(tf.matmul(valid_comp, weights_hidden1)  + biases_hidden1,8)),weights)+biases)
